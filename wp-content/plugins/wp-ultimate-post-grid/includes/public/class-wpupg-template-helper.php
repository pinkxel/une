...
class WPUPG_Template_Helper {
	...
	public static function get_label_container( $atts, $field ) {
		...
		if ( $atts['icon'] ) {
			...
			if ( $icon ) {
				$icon = '<span class="wpupg-item-icon wpupg-item-' . $field . '-icon">' . $icon . '</span> ';
			}
		}
		...
		$label = '';
		if ( $atts['label'] ) {
			$label = '<span class="wpupg-item-label wpupg-block-text-' . $atts['label_style'] . ' wpupg-item-' . $field . '-label">' . __( $atts['label'], 'wp-ultimate-post-grid' ) . $atts['label_separator'] . '</span>';
		}
